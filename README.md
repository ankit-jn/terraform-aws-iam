...
| Name | Description | Type | Default | Required | Example|
|:------|:------|:------|:------|:------:|:------|
| <a name="policies"></a> [policies](#policy) | List of IAM Policies configuration Map | `map` | `[]` | no | <pre>[<br>   {<br>     name = "full-stack-development-policy"<br>     description = "Full Stack Development policy"<br>     path = "/"<br>   },<br>   {<br>     name = "dashboard-policy"<br>     description = "Dashboard policy"<br>     path = "/"<br>   }<br>]<pre> |
| <a name="roles"></a> [roles](#role) | List of IAM Roles configuration Map | `map` | `[]` | no | <pre>[<br>   {<br>     name = "Project-Administrator"<br>     description = "Project Administrator Role"<br>     path = "/"<br>     policy_map = {<br>       policy_names = [<br>          "full-stack-development-policy"<br>       ]<br>       policy_arns = [<br>          "arn:aws:iam::aws:policy/AdministratorAccess",<br>       ]<br>     }<br>     tags = {}<br>   },<br>]<br><pre> |
| <a name="trust_account_ids"></a> [trust_account_ids](#input\_trust\_account\_ids) | Default Comma separated Account IDs to be trusted in case of Cross Account roles | `string` |  | no | `"xxxxxxxxxxxx,xxxxxxxxxxxx"` |
...
| <a name="trust_account_ids"></a> [trust_account_ids](#input\_trust_account_ids) | Comma separated Account IDs to be trusted in case of Cross Account roles. | `string` |  | no |  |
